...
from a Strapi v5 instance via the public REST API. This doc specifies the
collection type the CMS must expose so the frontend renders correctly.
...
API ID (singular): `article`  ·  API ID (plural): `articles`
...
| `body`         | Rich text (Blocks) **or** Rich text (Markdown) | yes | Main content. Both shapes are rendered by `src/components/blog/RichText.tsx`. |
...
In the Strapi admin, go to **Settings → Users & Permissions → Roles → Public**
and enable:
...
Strapi must allow requests from the frontend origin. In Strapi config
(`config/middlewares.ts`) add the production and local origins to the
`strapi::cors` middleware:

```ts
{
  name: 'strapi::cors',
  config: {
    origin: ['https://fuelcue.com', 'http://localhost:5173'],
  },
},
```
