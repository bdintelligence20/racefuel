 ...
 * product browser are one tap away rather than filling the screen. Reuses the
 * real generation (autoGeneratePlan), export (ExportModal) and route data.
 */
      ...
      <div className="max-w-2xl mx-auto w-full px-5 py-6">
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <button onClick={onShowMap} className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-[var(--color-border)] text-text-secondary hover:border-accent/40 hover:text-text-primary text-xs font-display font-semibold transition-colors">
              <MapIcon className="w-4 h-4" /> Map
            </button>
            <button onClick={resetRoute} className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-[var(--color-border)] text-text-secondary hover:text-text-primary text-xs font-display font-semibold transition-colors">
        ...
        {hasPlan ? (
          <>
            ...
            <div className="flex gap-2 mt-6">
              <button onClick={() => setExportOpen(true)} className="flex-[2] min-h-[54px] flex items-center justify-center gap-2 rounded-xl bg-accent text-white font-display font-bold text-sm hover:bg-accent-light active:scale-[0.99] transition-all">
                <Watch className="w-4 h-4" /> Export to watch
              </button>
              <button onClick={autoGeneratePlan} className="flex-1 min-h-[54px] flex items-center justify-center rounded-xl border border-accent text-accent font-display font-bold text-sm hover:bg-accent/[0.06] active:scale-[0.99] transition-all">
                Regenerate
              </button>
            </div>
          </>
        ) : (
          /* Route loaded, no plan yet */
          <div className="flex flex-col items-center text-center py-16">
            <h2 className="text-2xl font-display font-black text-text-primary tracking-tight">Ready to fuel this route</h2>
            <p className="text-sm text-text-secondary mt-1.5 max-w-xs leading-relaxed">
              We'll place fuel stops along the course and tell you what to take, and when.
            </p>
            <button onClick={autoGeneratePlan} className="mt-6 min-h-[54px] px-8 flex items-center justify-center rounded-xl bg-accent text-white font-display font-bold text-sm hover:bg-accent-light active:scale-[0.99] transition-all">
              Build my plan
            </button>
            <button onClick={onShowMap} className="mt-2 text-text-muted hover:text-text-primary text-xs font-display font-semibold py-2 transition-colors">
