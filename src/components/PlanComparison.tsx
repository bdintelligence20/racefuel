...
function MetricRow({
  ...
  return (
    <div className="grid grid-cols-4 gap-2 items-center py-2 border-b border-white/5">
      ...
      <div className="text-center">
        <span className="text-sm font-mono font-bold text-neon-blue">{recommended}{unit}</span>
      </div>
      <div className="text-center">
        <span className={`text-xs font-mono font-bold ${isBetter ? 'text-neon-green' : 'text-red-400'}`}>
          {diff > 0 ? '+' : ''}{pctDiff}%
        </span>
      </div>
    </div>
...
export function PlanComparison({ isOpen, onClose }: PlanComparisonProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-surface border border-white/10 w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              <div>
                ...
                <div className="h-6 bg-black/50 border border-white/10 relative rounded-sm overflow-hidden">
                  {currentSorted.map((p) => {
                    ...
                    return (
                      <div
                        ...
                        style={{ left: `${left}%`, backgroundColor: colorMap[p.product.color] || '#ff6b00' }}
              ...
              {recSorted.length > 0 && (
                <div>
                  <div className="text-[10px] text-neon-blue mb-1 uppercase">Recommended</div>
                  <div className="h-6 bg-black/50 border border-neon-blue/20 relative rounded-sm overflow-hidden">
                    {recSorted.map((p) => {
                      const left = (p.distanceKm / maxKm) * 100;
                      return (
                        <div
                          key={p.id}
                          className="absolute top-0 bottom-0 w-1 bg-neon-blue"
                          style={{ left: `${left}%` }}
                          title={`${p.product.name} @ ${p.distanceKm.toFixed(1)}km`}
                        />
                      );
          ...
          <div>
            <div className="grid grid-cols-4 gap-2 pb-2 border-b border-white/10">
              ...
              <div className="text-[10px] text-neon-blue uppercase tracking-wider text-center">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-black/30 border border-neon-blue/20 p-4">
              <div className="text-[10px] text-neon-blue uppercase tracking-wider mb-2">Recommended</div>
          ...
          {!recommendedMetrics && (
            <div className="bg-surfaceHighlight border border-white/10 p-4 text-center">
