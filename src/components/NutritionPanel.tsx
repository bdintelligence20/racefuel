...
export function NutritionPanel() {
  ...
  // Estimate hourly based on route time or default
    ...
    <aside className="w-80 bg-surface border-l border-white/10 flex flex-col h-full z-30 shadow-xl">
      ...
      <div className="p-4 bg-surfaceHighlight border-t border-white/10">
        ...
        <div className="w-full h-1.5 bg-black rounded-full overflow-hidden relative">
          {/* Target Zone */}
          <div className="absolute top-0 bottom-0 left-[50%] right-[25%] bg-white/5 z-0"></div>

          {/* Progress */}
          <div
            className={`h-full transition-all duration-500 z-10 ${
              carbsPerHour > 90 ? 'bg-red-500' : carbsPerHour < 60 ? 'bg-neon-blue' : 'bg-neon-green'
            }`}
            style={{
      ...
      <ProductDetailModal
        ...
        onAddToRoute={(product) => {
          // Add at a default position (middle of route) if route is loaded
          if (routeData.loaded) {
