...
import type { GpsPoint, NutritionPoint } from '../../context/AppContext';
import {
  buildCumDist,
  sampleAt,
  lerpAngle,
...
/**
 * Apply a uniform visual-scale to the cards / dots / route line. Used while recording
 * to HD: the modal preview wrapper is temporarily blown up to true HD pixels (e.g.
 * 1080×1920 for portrait) and then CSS-scaled down for display, but the icon-image
 * cards / circle-radius dots / line-width are in fixed pixel units and would otherwise
 * appear tiny relative to the bigger frame. Multiplying them by the same factor as the
 * wrapper expansion keeps the on-screen proportions unchanged in both the preview
 * (visually scaled-down) and the recording (full HD).
 *
 * scale = 1 → restore default sizes.
 */
export function setRenderScale(map: mapboxgl.Map, scale: number): void {
  if (map.getLayer(FLYOVER_CARDS_LAYER)) {
    map.setLayoutProperty(FLYOVER_CARDS_LAYER, 'icon-size', scale);
  }
  if (map.getLayer(FLYOVER_DOTS_LAYER)) {
    map.setPaintProperty(FLYOVER_DOTS_LAYER, 'circle-radius', [
      'interpolate', ['linear'], ['zoom'],
      10, 5 * scale,
      13, 8 * scale,
      16, 14 * scale,
    ...
    map.setPaintProperty(FLYOVER_DOTS_LAYER, 'circle-stroke-width', 2.5 * scale);
  }
  if (map.getLayer(FLYOVER_LINE_LAYER)) {
    map.setPaintProperty(FLYOVER_LINE_LAYER, 'line-width', 5 * scale);
  }
  if (map.getLayer(FLYOVER_GLOW_LAYER)) {
    map.setPaintProperty(FLYOVER_GLOW_LAYER, 'line-width', 16 * scale);
    map.setPaintProperty(FLYOVER_GLOW_LAYER, 'line-blur', 10 * scale);
  }
}
